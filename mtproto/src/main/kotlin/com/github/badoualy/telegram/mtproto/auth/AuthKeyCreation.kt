...
object AuthKeyCreation {
    ...
    private fun createAuthKeyInternal(dataCenter: DataCenter, tmpKey: Boolean, tag: String): AuthResult? {
        ...
        for (i in 0..AUTH_ATTEMPT_COUNT - 1) {
            try {
                ...
                logger.fine("Key created after ${i + 1} attempt in ${System.currentTimeMillis() - start} ms")
                connection = null
                return authResult
        ...
        logger.severe("Key creation failed $AUTH_ATTEMPT_COUNT times")
        connection = null
        return null
    }
    ...
    private fun lastStep(resPQ: ResPQ, newNonce: ByteArray, serverDhOk: ServerDhOk, @Suppress("UNUSED_PARAMETER") step4Duration: Long): Pair<ByteArray, Long> {
        ...
        for (i in 0..AUTH_RETRY_COUNT - 1) {
            ...
            if (result is DhGenOk) {
                ...
                logger.warning("Received ${result.javaClass.simpleName}")
    ...
    private fun createKey(tmpKey: Boolean): AuthResult {
        logger.fine("Attempting to create a " + (if (tmpKey) "temporary " else "permanent") + " Authorization Key")

        // Step 1
        val nonce = RandomUtils.randomInt128() // int128
        val resPQ = executeMethod(ReqPQ(nonce))
        logger.fine("Got resPQ with " + resPQ.fingerprints.size + " fingerprints")
        logger.fine("Step1 done")

        // Step 2
        val publicKey = J8Arrays.stream(Key.AVAILABLE_KEYS).filter { k ->
